...
imports = """import androidx.compose.foundation.gestures.detectDragGestures
...
new_canvas = """private data class TrendDataPoint(val label: String, val value: Int, val normalizedY: Float)
...
old_canvas_start = "private fun PriceTrendCanvas(timeframe: String) {"
old_canvas_end = """        }
    }
}"""

start_idx = content.find(old_canvas_start)
end_idx = content.find(old_canvas_end, start_idx) + len(old_canvas_end)

if start_idx == -1 or end_idx == -1:
    print("Could not find old canvas")
    sys.exit(1)

content = content[:start_idx] + new_canvas + content[end_idx:]

import_idx = content.find("import androidx.compose.ui.Alignment")
